...
class CustomWideResNet:
    ...
    def entry_block(input, num_filters):
        ...
        '''
        ## CONV -> BN -> ACT -> MaxPool
        x = SeparableConv2D(num_filters, (5,5), padding='same')(input)
    ...
    def residual_block(input_data, num_filters, stride):
        ...
        conv1 = SeparableConv2D(int(num_filters * 0.25), (1,1))(act1)

        # BN -> Act -> Conv2D(3,3)
        bn2 = BatchNormalization()(conv1)
        act2 = Activation('relu')(bn2)
        conv2 = SeparableConv2D(int(num_filters * 0.25), (3,3), strides=stride, padding='same')(act2)

        # BN -> Act -> Conv2D(1,1)
        bn3 = BatchNormalization()(conv2)
        act3 = Activation('relu')(bn3)
        conv3 = SeparableConv2D(num_filters, (1,1))(act3)
    ...
    def residual_block_reduce(input_data, num_filters, stride):
        ...
        bn2 = BatchNormalization()(conv1)
        act2 = Activation('relu')(bn2)
        conv2 = SeparableConv2D(int(num_filters * 0.25), (3, 3),
        ...
        short_cut = SeparableConv2D(num_filters, (1,1), strides=stride)(act1)
    ...
    def build_resnet(width, height, depth, num_classes, stages, filters):
        ...
        ## build entry block
        x = CustomWideResNet.entry_block(x, filters[0])
        ...
        for i in range(0, len(stages)):
            ...
            x = CustomWideResNet.residual_block_reduce(x, filters[i+1], stride)

            # depth of the residual block.
            for j in range(0, stages[i]-1):
               x = CustomWideResNet.residual_block(x, filters[i+1], (1,1))
